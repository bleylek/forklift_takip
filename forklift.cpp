...
const char* Version_Control = "Forklix_Esp32v.1.0.0";  //EKLENDİ
...
void setup() {
  ...
  ssid = preferences.getString("ssid", "RAK7268CV2_DB3F");        // RAK7268CV2_DB3F
...
void performOTA(const String& binUrl) {
  Serial.println("🔁 OTA işlemi başlatılıyor...");
  ...
  if (!http.begin(client, binUrl)) {
    Serial.println("❌ HTTPClient başlatılamadı.");
    return;
  }
  ...
  if (httpCode == HTTP_CODE_OK) {
    ...
    if (Update.begin(len)) {
      ...
      if (written == len && Update.end() && Update.isFinished()) {
        Serial.println("✅ OTA başarıyla tamamlandı. Cihaz yeniden başlatılıyor...");
        ESP.restart();
      } else {
        Serial.println("❌ OTA başarısız. Yazma veya bitirme sorunu.");
      }
    } else {
      Serial.println("❌ Update.begin başarısız.");
    }
  } else {
